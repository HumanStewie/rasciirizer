...
Everything should look MUCH better now, proper DDA algorithm is used. In the latest update, though, we have triangle rasterization! ~~No lighting and it is a bit buggy, still being worked on.~~ (We now have lighting!)
...
Of course, there are some more additions to the code, particularly in the projection in order to compensate for tall characters in terminal. ~~This part is still being improved.~~ (This part is now all good! Just multiplied x with inversed ratio of the screen)

The structure of our framebuffer is changed slightly, to fill the framebuffer, we iterate through the faces instead of all vertices. This means that we are applying rotation and translation multiple times to a vertex, which is inefficient.
...
Arrays were used for z-buffer and framebuffer, making it a faster a lookup and insertion.
