...
class Renderer {
   public:
    Renderer(const float depth, SDL_Window* window, int winWidth, int winHeight)
        : sdl_renderer_ { SDL_CreateRenderer(window, "software"),
                          &SDL_DestroyRenderer },
          m_depth { depth },
          m_wwidth { winWidth },
          m_wheight { winHeight },
    ...
    [[nodiscard]] SDL_Renderer* getRawRenderer() const
    {
   ...
   private:
                                           ...
                                           const float ooz) const
    {
        ...
        const int projX { static_cast<int>(
    ...
    [[nodiscard]] sgm::Vec<int, 2> getGridCoordinate(const int x,
                                                     const int y) const
    {
